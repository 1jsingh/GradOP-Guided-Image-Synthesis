...
class GradOPStroke2ImgPipeline(DiffusionPipeline):
    ...
    def prepare_latents(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None, latent=None):
        if not isinstance(image, (torch.Tensor, PIL.Image.Image, list)) and image is not None:
            raise ValueError(
        ...
        if latent is not None:
            init_latents = latent.clone().detach().to(device) / self.vae.config.scaling_factor
        elif image is None:
            height = width = 512
            init_latents = torch.zeros((1, self.unet.in_channels, height // 8, width // 8), device=device, dtype=self.unet.dtype)
    ...
    def text2img_prediction(self,prompt, num_images_per_prompt=1,generator=None, return_latents=False):
        """
        Compute pure text-based conditional generation to visualize the target subspace.
        """
        out = self(prompt=prompt,num_images_per_prompt=num_images_per_prompt, image=None, latent=None, generator=generator, return_latents=return_latents)
        return out

        
    def sdedit_img2img(self, prompt, image, strength=0.8, generator=None):
        """
    ...
    def gradop_plus_stroke2img(self, prompt, image, strength=0.8, num_iterative_steps=3, grad_steps_per_iter=12, generator=None):
        ...
        target = target.type(self.vae.dtype)

        # perform img2img guided synthesis using sdedit
        # generator = torch.Generator(device=device).manual_seed(seed)
        out = self(prompt=prompt, num_images_per_prompt=1, image=None, latent=None, strength=strength, generator=generator, 
                   gradop_plus=True, num_grad_steps=grad_steps_per_iter, threshold=num_iterative_steps,  lr=1e-1, target=target).images[0]
        return out
    ...
    def __call__(
        ...
        gradop_plus=False, num_grad_steps=12, threshold=2, target=None, lr=1e-1,
    ):
        r"""
        ...
        strength = 1.0 if (image is None and latent is None) else strength
        igrad_start = min(int(num_inference_steps*(1-init_image_strength)),num_inference_steps) #if igrad_in_domain else 0.0
        ...
        latents = self.prepare_latents(
            image, latent_timestep, batch_size, num_images_per_prompt, prompt_embeds.dtype, device, generator, latent=latent
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # perform gradop_plus optimization by iteratively modifying the latents during reverse diffusion
                if gradop_plus and i < threshold + igrad_start and i >= igrad_start and i < len(timesteps)-1 and target is not None:
                    ...
                    if True:
                        ...
                        latents = self.scheduler.add_noise(latents.detach(), noise, t_enc).to(device)
